...
- `led.rs` — WS2812 NeoPixelチェーン(PIO1 + DMA_CH2、GPIO15、`config::PEOPLE`と同順)を所有。`LED_CHANNEL`のイベントを人ごとの押下状態に取り込む(`Pressed`で押下、`Cancelled`で取り消し、`Sync`(`status_poll.rs`から)で全員分を一括反映 — サーバーの日次リセット後にLEDを消す役目もこれが担う)。書き込みは`config::NEOPIXEL_FRAME_INTERVAL_MS`(100ms、10fps)の固定周期で、変化がなくても毎フレーム5個ぶんを書く(`with_deadline`で次のフレームの時刻までイベントを待つ。イベントで書き込みが早まることはなく、変化は1フレーム以内に反映される)。色や点滅は決めず、`led_pattern::render`に任せる
- `led_pattern.rs` — NeoPixelの「見え方」だけを決める。`render(&pressed, now_ms) -> [RGB8; NUM_PEOPLE]`は、押下状態と時刻だけから1フレームぶんの色を返す純粋関数(ハードウェア非依存)。現状は、押下済みの人が人ごとの色(表`PRESSED_COLORS`、`config::PEOPLE`と同順)で点灯し続け、押していない人は白で6秒周期(`BREATH_PERIOD_MS`)でだんだん明るく/暗くを繰り返す(呼吸の位相は`now_ms`から計算)。色や周期などの見え方に関する定数は`config.rs`ではなくこのファイルに置き、見え方を変えるときは、このファイルだけを変更する
...
firmware/src/{main,secrets.rs.example,config,irqs,events,debounce,net,wifi,outbox,buttons,occupancy,http_client,led,led_pattern,status_poll,audio,waveform,music}.rs
