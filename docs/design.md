...
- `led_pattern.rs` — NeoPixelの「見え方」だけを決める。`render(&pressed, now_ms) -> [RGB8; NUM_PEOPLE]`は、押下状態と時刻だけから1フレームぶんの色を返す純粋関数(ハードウェア非依存)。現状は、押下済みの人が緑(0,40,0)で`NEOPIXEL_BLINK_HALF_PERIOD_MS`(250ms)ごとに点灯/消灯を繰り返し、押していない人は消灯(点滅の位相は`now_ms`から計算)。見え方を変える(常時点灯、呼吸、人ごとの色など)ときは、このファイルだけを変更する
