...
ボタンの長押し(2秒以上)による、その人の今日の押下の取り消し。押下と同様に、未送信の間は保持され、2xxが返るまで再送されます。人ごとに未送信として保持されるのは最後の操作(押下か取り消し)だけなので、未送信の押下の後に取り消しがあれば、取り消しだけが送られます。サーバー側は冪等に扱います(押していない人を取り消しても何も起きません)。4xxが返った場合は破棄します。

取り消すと、その人の`last_pressed_at`は`null`、`pressed_today`は`false`に戻り、`GET /api/led-state`の該当の文字も`0`になります(ファームウェアは次の同期で反映しますが、取り消した基板自身は長押しの時点でLEDを消しています)。
...
レスポンス `200 OK`:
```json
{ "status": "ok", "person": "grandpa" }
```

レスポンス `400 Bad Request`(`person`が未知または欠落している場合): `POST /api/press`と同じ。
